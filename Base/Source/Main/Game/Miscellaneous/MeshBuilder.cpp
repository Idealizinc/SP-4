...
Mesh* MeshBuilder::GenerateAvatar(const std::string &meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.pos.Set(-0.5f * length, -1.f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 0);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string& file_path, std::vector<unsigned char>& heightMap)
{
	...
	{
		for (int x = 0; x < (int)terrainSize; ++x)
		{
			...
			if (z == 0)
			{
				v.normal.z = scaledHeight - (heightMap[(z + 1) * terrainSize + x] / SCALE_FACTOR);
			}
			else if (z == terrainSize - 1)
			{
				v.normal.z = (float)(heightMap[(z - 1) * terrainSize + x] / SCALE_FACTOR) - scaledHeight;
			}
			else
			{
				v.normal.z = ((scaledHeight - (float)heightMap[(z + 1) * terrainSize + x] / SCALE_FACTOR) + ((float)heightMap[(z - 1) * terrainSize + x] / SCALE_FACTOR - scaledHeight)) * 0.5f;
			}
			if (x == 0)
			{
				v.normal.x = scaledHeight - (heightMap[z * terrainSize + (x + 1)] / SCALE_FACTOR);
			}
			else if (x == terrainSize - 1)
			{
				v.normal.x = (float)(heightMap[(z)* terrainSize + (x - 1)] / SCALE_FACTOR) - scaledHeight;
			}
			else
			{
				v.normal.x = ((scaledHeight - (float)heightMap[(z)* terrainSize + (x + 1)] / SCALE_FACTOR) + ((float)heightMap[(z)* terrainSize + (x - 1)] / SCALE_FACTOR - scaledHeight)) * 0.5f;
			}
			v.normal.Set(v.normal.x, 1, v.normal.z);
			v.normal.Normalize();
			vertex_buffer_data.push_back(v);
		}
	}
	...
	for (int z = 0; z < (int)terrainSize - 1; ++z) {
		for (int x = 0; x < (int)terrainSize - 1; ++x) {
			//Triangle 1
