...
public:
	...
	virtual void Attack(CharacterEntity* CE)
	{
 		...
 		for (int i = 0; i < NumberOfBullets; ++i)
		{
			...
			Bullet->SetParameters("quad", 1, CE->GetPosition() + Vector3(0, CE->GetDimensions().y* 0.5f) + Velocity * 0.01f, Vector3(CE->GetDimensions().x * 0.025f, 1, CE->GetDimensions().y * 0.1f), Velocity, 0, Vector3(0, 1, 0));
